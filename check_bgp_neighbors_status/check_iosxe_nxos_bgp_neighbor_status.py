...
def check_show_bgp(switch):
    ...
    myparamiko.send_command(shell, 'show ip bgp summary | begin ^Neighbor')
    myparamiko.send_command(shell, '\n')
    myparamiko.send_command(shell, 'show bfd neighbors')
    myparamiko.send_command(shell, '\n')
    myparamiko.send_command(shell, 'show logging | inc BGP-5-ADJCHANGE|BFD') 
    myparamiko.send_command(shell, '\n')    
    time.sleep(1)
    myparamiko.send_command(shell, 'show run | sec bgp')
    time.sleep(1)
