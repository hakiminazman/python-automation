...
for ip_address in iosxe_intf:
    ...
    time.sleep(2)
    iosxe_device.open()
    run_get_interfaces_ip = iosxe_device.get_interfaces_ip()
    print(json.dumps(run_get_interfaces_ip, sort_keys=True, indent=4))
    file_name = f'{ip_address}-get_interfaces_ip_{hour}{minute}_{day}-{month}-{year}.txt'
    with open(file_name, 'a') as outfile:
        json.dump(run_get_interfaces_ip, outfile, indent=4)
...
for ip_address in nxos_intf:
    ...
    time.sleep(2)
    nxos_device.open()
    run_get_interfaces_ip = nxos_device.get_interfaces_ip()
    print (json.dumps(run_get_interfaces_ip, sort_keys=True, indent=4))
    file_name = f'{ip_address}-get_interfaces_ip_{hour}{minute}_{day}-{month}-{year}.txt'
    with open(file_name, 'a') as outfile:
        json.dump(run_get_interfaces_ip, outfile, indent=4)
