#This script can be used to capture the post-logs from Internet routers (ASR-1002-HX)
...
def asr_post_logs(device):
    ...
    for cmd in show_cmd:
        ...
        filename = f'{hostname}-post_logs_{hour}_{day}-{month}-{year}.txt'
        with open(filename, 'a+') as asr_post_logs:
            asr_post_logs.write('\n')
            asr_post_logs.write(hostname + '#' + cmd)
            asr_post_logs.write('\n')
            asr_post_logs.write(output)
            asr_post_logs.write('\n')
...
for ip in devices:
    device = {
        ...
        'timeout': 150,     # ssh timeout 
        'verbose': True  # optional, default False
    }
    # creating a thread for each router that executes the backup function
    th = threading.Thread(target=asr_post_logs, args=(device,))
