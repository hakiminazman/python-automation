# This script can be used to capture the pre-logs from DC Agg switches
...
# this function nx_pre_logss the config of a switch
# this is the target function which gets executed by each thread
def nx_pre_logs(switch):
    ...
    file_name = f'{switch["switch_hostname"]}-pre-logs_{day}-{month}-{year}.txt'
...
switch1 = {'switch_hostname':'rpdcabgr5w-0309-002.network.paynet.my', 'switch_port': '22', 'user':user_name, 'passwd':user_pwd}
switch2 = {'switch_hostname':'rpdcabgr5w-0310-001.network.paynet.my', 'switch_port': '22', 'user':user_name, 'passwd':user_pwd}
...
for switch in switches:
    # creating a thread for each switch that executes the nx_pre_logs function
    th = threading.Thread(target=nx_pre_logs, args=(switch,))
