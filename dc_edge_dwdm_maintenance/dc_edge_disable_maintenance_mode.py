# This script can be used to disable BGP GSHUT and OSPF Max-Metric Feature
...
def nx_disable_maintenance(switch):
    ...
    myparamiko.send_command(shell, 'no graceful-shutdown activate')  #Disable BGP graceful shutdown (GSHUT) feature  
    myparamiko.send_command(shell, 'exit')
    myparamiko.send_command(shell, 'router ospf IPN')    
    myparamiko.send_command(shell, 'vrf IPN')    
    myparamiko.send_command(shell, 'no max-metric')    #Disable OSPF max-metric feature
    ...
    file_name = f'{switch["switch_hostname"]}-disable_maintenance_{day}-{month}-{year}.txt'
...
for switch in switches:
    # creating a thread for each switch that executes the nx_disable_maintenance function
    th = threading.Thread(target=nx_disable_maintenance, args=(switch,))
